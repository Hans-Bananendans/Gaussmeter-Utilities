...
class CallbackTaskSynchronous(PyDAQmx.Task):
    def __init__(self, dev_name=None, data_len=1000):
        PyDAQmx.Task.__init__(self)
        if dev_name is None:
            dev_name = "Dev1"
        self._data = zeros(1000)
        self.read = PyDAQmx.DAQmxTypes.int32()
        self.CreateAIVoltageChan(dev_name+"/ai0","",PyDAQmx.DAQmx_Val_RSE,-1.25,1.25,PyDAQmx.DAQmx_Val_Volts,None)
        self.CfgSampClkTiming("",10000.0,PyDAQmx.DAQmx_Val_Rising,PyDAQmx.DAQmx_Val_ContSamps,1000)
    ...
    def EveryNCallback(self):
        with self._data_lock:
            self.ReadAnalogF64(1000,10.0,PyDAQmx.DAQmx_Val_GroupByScanNumber,self._data,1000,byref(self.read),None)
